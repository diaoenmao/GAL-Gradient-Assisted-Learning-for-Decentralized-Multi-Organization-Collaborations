...
class MIMIC(Dataset):
    ...
    def make_data(self):
        data = pd.read_csv(os.path.join(self.raw_folder, 'MIMIC_cleaned_data.csv'), delimiter=',')
        data = data.to_numpy()
        ...
        train_target, test_target = data[:split_idx, [-1]].astype(np.float32), data[split_idx:, [-1]].astype(np.float32)
